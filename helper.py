...
class MenuView(View):
    def __init__(self, bot_config: BotConfig, state: BotState):
        ...
        cmds = [
            ...
            ("📜", "👤", "!rules", discord.ButtonStyle.success)
        ]
        for e, l, c, s in cmds: 
            self.add_item(MenuButton(label=l, emoji=e, command=c, style=s, bot_config=bot_config, state=state))
...
class BotHelper:
    ...
    async def send_main_menu(self, target: Any) -> None:
        """Sends the main interactive help menu embed with buttons."""
        try:
            help_description = """
**Info** ----------- Server Info/Rules
**Times** --------- Top 10 VC Times
**Rules** ---------- Show Server Rules
**!commands** --- List All Commands
"""
            embed = discord.Embed(title="👤  Server Controls  👤", description=help_description, color=discord.Color.blue())
            destination = target.channel if hasattr(target, 'channel') else target
            if destination and hasattr(destination, 'send'):
                await destination.send(embed=embed, view=MenuView(self.bot_config, self.state))
        except Exception as e:
            logger.error(f"Error in send_main_menu: {e}", exc_info=True)
    ...
    async def show_timeouts(self, ctx) -> None:
        ...
        async with self.state.moderation_lock:
            disabled_user_ids = list(self.state.command_disabled_users)
